...
export function inject(container: Container) {
    container.bind<MovieDatabase>(MovieDatabaseType).to(MongoMovieDatabase).inSingletonScope()
    container.bind<Controller>(ControllerType).to(MovieController).inSingletonScope()
}
